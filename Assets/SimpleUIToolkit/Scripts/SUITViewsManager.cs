...
namespace SUIT
{
    ...
    public sealed class SUITViewsManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            _viewsHistoryConfig = SUITConfigProvider.ViewsHistoryConfig;
        ...
        private void HandleShowPreviousView()
        {
            if (!_viewsHistoryConfig.EnableViewsHistory)
            {
                Logger.LogWarning($"The views history feature is currently disabled. To enable it, navigate to {Constants.ConfigName} and select \"Enable Views History\" to true");
                return;
            }
        ...
        {
            if (Application.isPlaying || !SUITConfigProvider.GeneralConfig.EnableAutoRebuildViewsTree)
                return;
            _lcaTree = LCATreeBuilder.Build(this);
        }
#endif
